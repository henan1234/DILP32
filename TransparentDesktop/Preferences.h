...
{
public:
	static void DejaEdite( BOOL Deja );
	static BOOL DejaEdite( void );
	// Importe un fichier image
	static BOOL ImportPage( const char * PageFileName );

	// Protection des pages par mot de passe
	static void Protege( int NoPage, BOOL bProtect );
	...
	// Image courante
	static BOOL SetImageCourante( int No );
	static int GetNoImageCourante( void );
	static CString FichierImageCourante( void );
	...
	static	COLORREF	CouleurTransparente( void ) { return _CouleurTransparente ; } ;
...
private:
	...
	static CStringArray _FichiersImages;
