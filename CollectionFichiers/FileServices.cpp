...
BOOL CFileServices::DeleteDirectory( const TCHAR * RootPath )
	{
//	LogFile.Write( "Deleting directory ", RootPath ) ;
	...
	while ( Continue )
		...
		if ( _tcscmp( FindData.cFileName, TEXT(".") ) && _tcscmp( FindData.cFileName, TEXT("..") ))
			...
			if ( FindData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY  )
				...
				{
				//LogFile.Write( "Deleting file ", CompletePath ) ;
				// Delete the file
...
//	LogFile.Write( "Removing directory ", RootPath ) ;
...
BOOL CFileServices::CopyDirectory( const TCHAR * pSource, const TCHAR * pDest )
	{
//	LogFile << "Copying directory " << pSource << " to " << pDest << CLogFile::endline ;
...
CString	CFileServices::GetTempDirectory()
	...
	if ( IsWritableDirectory( Path ))
		{
//		LogFile << "Using " << Path << " as temporary directory" << CLogFile::endline ;
		return Path ;
		}
	...
	if ( IsWritableDirectory( TempUnderWin ))
		{
//		LogFile << "Using " << TempUnderWin << " as temporary directory" << CLogFile::endline ;
		return TempUnderWin ;
		}
