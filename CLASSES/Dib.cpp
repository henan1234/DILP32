...
void	ControleDimensions( int & Depart, int & Taille, int TailleBitmap )
	{
	if ( Depart >= TailleBitmap )
		{
		Taille = 0 ;
		return ;
		}
	...
	if ( Depart + Taille > TailleBitmap )
		{
		Taille = TailleBitmap - Depart ;
		}
	}
...
void* CDIB::GetPixelAddress(int x, int y)
    ...
    if ((x >= DibWidth()) || (y >= DibHeight()) || ( x < 0) || (y < 0)) 
		{
                    ...
                    COLORREF clrTrans)
{
    ...
    ASSERT(pdibDest);
	// Teste si le rectangle sort de la destination
	ControleDimensions( xd, w, pdibDest->DibWidth()) ;
	ControleDimensions( yd, h, pdibDest->DibHeight()) ;
	ControleDimensions( xs, w, DibWidth()) ;
	ControleDimensions( ys, h, DibHeight()) ;
...
void CDIB::FillBits( int x, int y, int w, int h, COLORREF Color)
{
	...
	ControleDimensions( x, w, DibWidth()) ;
	ControleDimensions( y, h, DibHeight()) ;
                       ...
                       COLORREF clrTrans)
{
	...
	ControleDimensions( xd, wd, pdibDest->DibWidth()) ;
	ControleDimensions( yd, hd, pdibDest->DibHeight()) ;
	ControleDimensions( xs, ws, DibWidth()) ;
	ControleDimensions( ys, hs, DibHeight()) ;
