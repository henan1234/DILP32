...
static void _cdecl Error( const TCHAR * pMessage, ... )
	{
	char Message[2048] ;
    va_list va;
...
CDIBSurface::CDIBSurface()
    ...
    if (m_bUseCreateDIBSection) 
		...
		if (m_bUseWinG) 
			...
			{
			Error( "Could not crearte dib surface, not supported by your system" ) ;
			}
	}
...
BOOL CDIBSurface::Create(int cx, int cy, HPALETTE hPal)
    ...
    if (hPal) 
        ...
        ASSERT(pLP);
		if ( ! pLP )
			Error( "Could not allocate memory to store dib surface. Not enough memory" ) ;
		
        pLP->palVersion = 0x300;
        pLP->palNumEntries = (short)iColors;
    ...
    if (m_bUseCreateDIBSection) 
        ...
        m_hbmBuffer = CreateDIBSection(dc.GetSafeHdc(),
			...
			0);
        if (hPalOld) 
			::SelectPalette(dc.GetSafeHdc(), hPalOld, FALSE);
        ASSERT(m_hbmBuffer);
		
        ASSERT(pBits);
		if ( ! pBits )
			Error( "Could not create DIB section. Not enough memory" ) ;
...
{
    BitBlt(pDC, 0, 0, GetWidth(), GetHeight(), 0, 0);
}
...
{
	if ( ! pDC )
		Error( "Could not draw image. No source image given" ) ;
...
void CDIBSurface::StretchBlt(CDC* pDC, int xd, int yd, int wd, int hd,
	...
	{
	if ( ! pDC )
		Error( "Could not stretch image. No source image given" ) ;
...
BOOL CDIBSurface::Load(const char* pszFileName/*=NULL*/)
{
    ...
    if (!Create(dib.GetWidth(), dib.GetHeight(), &pal)) 
		return FALSE;
...
BOOL CDIBSurface::MapColorsToPalette(HPALETTE hPal)
{
    ...
    if (!CDIB::MapColorsToPalette(hPal)) 
		return FALSE;
