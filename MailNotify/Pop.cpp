...
BOOL CPop::Connect( const CString & Host, int Port, const CString & User, const CString & Password)
{
	char	buf [512];
...
// Header
CString  CPop::RetrieveHeader(int  MsgNumber)
{
	char	buf [1024];

	wsprintf (buf, "TOP %d 10\r\n",MsgNumber );	
	m_PopServer.Send(buf, strlen (buf)); 
	if (CheckResponse(RETR_HEADER)==FALSE)
		return "" ;
	else
...
BOOL CPop::Connect()
{
	Connect(m_Host, m_Port, m_User, m_Password);
	return TRUE;
}
