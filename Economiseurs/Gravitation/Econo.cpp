...
BOOL ShareWare()
{
	...
	if ( ptm->tm_year >= 98 && ptm->tm_mon >= 1 && ptm->tm_mday >= 3 )
		...
		if ( ! fChildPreview )
			...
			MessageBox( 0, 
						"This is a shareware program. You had time to test it, it is now time for you to register ( see the configuration box ).",
						//"Ceci est un ShareWare, il est temps de vous enregistrer ( voyez la boîte de configuration ).",
